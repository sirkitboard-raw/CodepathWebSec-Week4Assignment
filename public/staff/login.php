...
if(is_post_request()) {
  ...
  if(isset($_POST['username'])) { $username = addslashes(h($_POST['username'])); }
  if(isset($_POST['password'])) { $password = addslashes(h($_POST['password'])); }
  ...
  if (empty($errors)) {
    ...
    if($user) {
      ...
      // No username found
      $errors[] = "Username does not exist"; // TODO write an error message
    }
  }
}
